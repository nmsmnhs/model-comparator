...
#Load dataset (from Flask)
#target_col is from frontend dropdown
def get_stats(df, target_col):
    ...
    best_models["best_precision_model"] = [best_precision_model[0], best_precision_model[1]['precision']]
    best_models["best_accuracy_model"] = [best_model[0], best_model[1]['accuracy']]
    if roc_curves:
        best_auc_model = max(
            roc_curves.items(),
            key=lambda x: x[1]["auc"]
        )
        best_AUC = round(best_auc_model[1]["auc"], 4)
        best_models["best_auc_model"] = [best_auc_model[0], best_AUC]   
